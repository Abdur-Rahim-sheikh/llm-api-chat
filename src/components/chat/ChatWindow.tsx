  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center px-8">
      ...
      <div>
        ...
        <p className="text-zinc-600 text-sm mt-1">Create a new chat from the sidebar to get started.</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center px-8">
      <div className="w-16 h-16 rounded-2xl bg-amber-950/40 border border-amber-800/40 flex items-center justify-center">
        <svg viewBox="0 0 24 24" className="w-8 h-8 text-amber-500" fill="none" stroke="currentColor" strokeWidth="1.5">
      ...
      <div>
        ...
        <p className="text-zinc-500 text-sm mt-1">
          Add your {provider === "openai" ? "OpenAI" : "Gemini"} API key in Settings to start chatting.
        </p>
      </div>
    </div>
  );
...
export function ChatWindow({ conversation }: ChatWindowProps) {
  ...
  const { messages, isLoading, error, sendMessage } = useChat(conversation, settings);
      ...
      {!conversation ? (
        ...
        <>
          <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6">
            ...
            {isLoading && <TypingIndicator provider={settings.active_provider} />}
            {error && (
